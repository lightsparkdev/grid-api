...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw GridInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw GridInvalidDataException("`$name` is not set")
            is JsonNull -> throw GridInvalidDataException("`$name` is null")
            else -> throw GridInvalidDataException("`$name` is invalid, received $this")
        }
