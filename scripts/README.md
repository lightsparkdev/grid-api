...
Pass `--compressed`. The enclave only accepts a compressed public key
(`02`/`03` prefix) in the OTP bundle. With the uncompressed default, step 3.4
fails with `{"code":"INVALID_INPUT","reason":"Invalid wallet request."}`, even
...
| `400 INVALID_INPUT` `Invalid wallet request.` on the first `/verify` call | Grid returns this whenever the wallet provider rejects the request, so it has more than one cause. First check the OTP code for typos. If the code is correct, check that `PUB_HEX` starts with `02` or `03`. An uncompressed key (`04` prefix) causes this error; regenerate it with `gen-keypair --compressed` (step 3.1). |
