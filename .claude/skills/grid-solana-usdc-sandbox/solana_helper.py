#!/usr/bin/env python3
"""Solana devnet CLI for Grid USDC sandbox testing.

Subcommands:
  wallet-address              Print public key of loaded devnet keypair
  sol-balance [--address]     Print SOL balance
  usdc-balance [--address]    Print USDC balance (raw + human-readable)
  send-usdc --to --amount     Send USDC on devnet (amount in micro-USDC)
  airdrop-sol [--amount]      Request devnet SOL airdrop
"""
...
def load_keypair(creds_path=None):
    ...
    with open(creds_path) as f:
        creds = json.load(f)
    secret_key = creds.get("solanaDevnetPrivateKey")
    if not secret_key:
        print(json.dumps({"error": "solanaDevnetPrivateKey not found in ~/.grid-credentials"}))
        sys.exit(1)
...
def cmd_usdc_balance(args):
    client = get_client()
    mint_str = args.mint or DEVNET_USDC_MINT
    if args.address:
        pubkey = Pubkey.from_string(args.address)
    else:
        kp = load_keypair()
...
def main():
    parser = argparse.ArgumentParser(description="Solana devnet helper for Grid USDC sandbox testing")
    sub = parser.add_subparsers(dest="command")
    sub.required = True

    sub.add_parser("wallet-address", help="Print public key of loaded devnet keypair")
    ...
    usdc_bal = sub.add_parser("usdc-balance", help="Print USDC balance on devnet")
    usdc_bal.add_argument("--address", help="Address to check (default: own wallet)")
    usdc_bal.add_argument("--mint", help="Token mint address (default: devnet USDC)")

    send = sub.add_parser("send-usdc", help="Send USDC on devnet")
    ...
    send.add_argument("--mint", help="Token mint address (default: devnet USDC)")
