#!/usr/bin/env python3
"""Arbitrum One CLI for Grid USDC and USDT testing.

Supports both Arbitrum Sepolia testnet (default) and Arbitrum One mainnet (--mainnet flag).
...
# Contracts match sparkcore/sparklib/money/evm.py, which is what Grid indexes.
NETWORKS = {
    "testnet": {
        ...
        "tokens": {
            ...
            "usdt": "0xc6d73dc581a37fece4fa24f6e0dfb2fea406d534",
        },
        "cred_key": "arbitrumTestnetPrivateKey",
    ...
    "mainnet": {
        ...
        "cred_key": "arbitrumMainnetPrivateKey",
        "name": "Arbitrum One",
        "priority_fee_gwei": 0.01,
    },
}
...
def cmd_send_token(asset):
    def run(args):
        ...
        # that true even when base fee is far below the priority tip.
        ...
        transfer = token.functions.transfer(recipient, amount)

        # Arbitrum folds the L1 data-posting cost into the gas limit, so a
        # fixed 100k limit that works on other EVM chains can run out of gas
        # here whenever L1 fees spike.
        gas_limit = int(transfer.estimate_gas({"from": acct.address}) * 1.5)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Arbitrum helper for Grid USDC/USDT testing")
    parser.add_argument("--mainnet", action="store_true", help="Use Arbitrum One mainnet instead of Arbitrum Sepolia")
