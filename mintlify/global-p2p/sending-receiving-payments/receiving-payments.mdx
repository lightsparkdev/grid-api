...
If an incoming payment cannot be completed after it is accepted, Grid initiates a refund automatically. The transaction moves to `FAILED`, and you'll receive `INCOMING_PAYMENT.REFUND_PENDING` followed by `INCOMING_PAYMENT.REFUND_COMPLETED` or `INCOMING_PAYMENT.REFUND_FAILED`. The transaction's `refund` object tracks the refund status and reference.

```json
{
  "id": "Webhook:019542f5-b3e7-1d02-0000-000000000011",
  "type": "INCOMING_PAYMENT.REFUND_COMPLETED",
  ...
  "data": {
    ...
    "refund": {
      "reference": "REF-123456789-REFUND",
      "initiatedAt": "2023-08-15T14:35:00Z",
      "settledAt": "2023-08-15T14:40:00Z",
      "status": "COMPLETED",
...
<Note>
  For the `refund` object field reference, see the [Transaction Lifecycle](/platform-overview/core-concepts/transaction-lifecycle#refund-object) guide.
</Note>
