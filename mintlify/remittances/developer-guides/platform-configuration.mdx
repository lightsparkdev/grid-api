...
These fields must be supplied when creating or updating a user via the `POST /users` or `PATCH /users/{userId}` endpoints if that user is expected to use the specified currency. Refer to the [Configuring Users](/remittances/developer-guides/configuring-users) guide for more details on how this impacts user setup.
