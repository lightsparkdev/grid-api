...
sequenceDiagram
    ...
    Client->>Grid: PUT /config (set domain, webhook URL)
    ...
    alt Payment approved
        Client-->>Grid: HTTP 200 OK (approve payment, provide PII if requested)
...
Configure your platform settings (if you haven't already in the onboarding process). See the [Platform Configuration](/api-reference/platform-configuration/get-platform-configuration) guide for more details.
...
See the [Configuring Users](/remittances/developer-guides/configuring-users) guide for more details.

## Step 3: Webhook setup (one-time setup)

Configure your webhook endpoints to receive notifications about incoming payments. You'll need to implement the webhook endpoints on your server. Remember to validate webhook signatures to ensure they are authentic. See the [Webhook Verification](/remittances/platform-tools/webhooks) guide for more details.
...
sequenceDiagram
    ...
    Note over Sender, Grid: Payment initiated by sender
    Sender->>Grid: Lnurlp request for UMA address
    Note over Grid: Resolves the UMA address to a registered user
    Grid->>Sender: Lnurlp response with currencies and requested payer fields
    Sender->>Grid: Payreq request with payer information and amount
    Grid->>Client: Webhook: INCOMING_PAYMENT (PENDING)
    
    alt Payment approved
        Client-->>Grid: HTTP 200 OK (approve payment, provide recipient PII if requested)
        Grid-->>Sender: Payreq response with payee info and a Lightning Invoice
        ...
        Client-->>Grid: HTTP 403 Forbidden with rejection reason
        Grid->>Sender: Failed payreq response.
    end
```
