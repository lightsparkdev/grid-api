...
<Tabs>
  ...
  <Tab title="IBAN (Europe)">
    ...
    Grid derives `swiftCode` from the IBAN when possible. If automatic derivation
    is unavailable, provide it in the request. Responses may include the derived
    value.
  </Tab>
...
<Tabs>
  <Tab title="As Destination (Payout)">
    Send funds from internal account to external account. Use `/quotes` for both
    same-currency and cross-currency payouts; `immediatelyExecute` moves the funds
    in the same request:
    ...
    Omit `immediatelyExecute` to review the rate and fees first, then call
