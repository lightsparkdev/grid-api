...
Same-currency transfers use `/quotes` like every other transfer. With
`immediatelyExecute` set to `true`, the quote is created and executed in one
request.

<Note>
  The `/transfer-out` and `/transfer-in` endpoints are deprecated. They still work
  and their request and response shapes are unchanged, but new integrations should
  use `/quotes`.
</Note>
...
The transaction referenced by `transactionId` follows the same lifecycle as any
other outgoing transaction.
