    ...
    <table className="rate-explorer-table">
      <thead>
        <tr>
          ...
          {cols.map((name, i) => <th key={i}>{name}</th>)}
      ...
      <tbody>
        {CURRENCIES.filter(c => c.code !== 'USD').slice(0, 40).map((c) => (
          <tr key={c.code} className="rate-explorer-row">
            ...
            <td className="rate-explorer-provider-cell"><div className="rate-explorer-skeleton-bar" /></td>
...
export const StaticPlaceholder = () => (
  <div id="rate-explorer-static" className="not-prose">
    ...
    <SkeletonTable cols={['Stripe', 'Airwallex', 'Wise', '', '', 'Avg Bank']} />
  </div>
);
...
export const RateExplorer = () => {
  ...
  const [competitorDone, setCompetitorDone] = React.useState(false);
    ...
    <div ref={containerRef}>
      <div className="rate-explorer not-prose">
        ...
        <div className="rate-explorer-disclaimer">
          Grid rates reflect standard published pricing (not personalized quotes). Wise and dynamic provider rates are live via public comparison data and include all fees. Stripe and Airwallex costs are based on published pricing schedules, not live API data. Avg Bank is the average of bank-type providers from comparison data. Actual rates may vary. Get real-time Grid quotes via the <a href="/platform-overview/core-concepts/quote-system">Quote API</a>.
        </div>
      </div>
    </div>
  );
