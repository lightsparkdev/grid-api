...
function displayPaymentInstructions(quote) {
  ...
  if (!quote.paymentInstructions?.length) {
...
EUR / USDC is gated on its swap leg. When it applies, the transaction comes back
`PENDING_AUTHORIZATION` with an `scaChallenge` that you authorize before the
