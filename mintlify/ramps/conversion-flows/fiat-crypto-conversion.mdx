...
function displayPaymentInstructions(quote) {
  // EU customers only: an SCA-gated realtime-funding quote comes back
  // PENDING_AUTHORIZATION with paymentInstructions withheld until its
  // scaChallenge is authorized (see "Strong Customer Authentication" below).
  // Authorize first, then display — don't read paymentInstructions while pending.
  if (quote.status === "PENDING_AUTHORIZATION") {
    return { pendingAuthorization: true, scaChallenge: quote.scaChallenge };
  }
...
money movements with Strong Customer Authentication, and a conversion out of
EUR / USDC is gated on its swap leg. When it applies, the quote comes back
`PENDING_AUTHORIZATION` carrying an `scaChallenge` that you authorize before the
...
<Accordion title="Settlement times by account type">
```javascript
const settlementTimes = {
  USD_ACCOUNT: "1-3 business days (ACH)",
  EUR_ACCOUNT: "Seconds to 1 business day (SEPA Instant / SEPA)",
  BRL_ACCOUNT: "Instant (PIX)",
  SWIFT_ACCOUNT: "1-3 business days",
