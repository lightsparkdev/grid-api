...
**Required fields:** `fullName`, `birthDate`, `address`

**Conditionally required:** `email` and `phoneNumber` are required in regions that verify contact information before identity verification (e.g., the EU).
...
**Required fields:** `customerType`, plus `businessInfo.legalName`, `businessInfo.country`, `businessInfo.taxId`, and `businessInfo.incorporatedOn`. Most providers also require `address` for KYB.
