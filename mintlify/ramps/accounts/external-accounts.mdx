...
- **Traditional bank accounts**: `USD_ACCOUNT`, `EUR_ACCOUNT`, `MXN_ACCOUNT`, `BRL_ACCOUNT`,
  `INR_ACCOUNT`, and `SWIFT_ACCOUNT` for corridors with no local rail
...
### Example: SWIFT account for off-ramp

For destinations where Grid has no local payment rail, use a `SWIFT_ACCOUNT` to pay out over
international wire. `currency` is the currency the destination account holds, and `country` is
...
curl -X POST 'https://api.lightspark.com/grid/2025-10-13/customers/external-accounts' \
  ...
  -d '{
    "customerId": "Customer:019542f5-b3e7-1d02-0000-000000000001",
    "currency": "USD",
    "platformAccountId": "user_swift_001",
    "accountInfo": {
...
<Note>
  Provide `accountNumber` for most corridors. For IBAN-only corridors such as GB and BR, provide
  `iban` instead.
</Note>
