...
<Note>
In sandbox, you can use special account number patterns to test different scenarios. The **last 3 digits** determine the behavior, and the meaning depends on how the account is used. This account is the destination of the off-ramp quote below, so the quote patterns apply: **002** (quote execution failed), **003** (long payment), **004** (counterparty delivery failed), **005** (receiving bank returned the payment), **006** (user cancellation), **007** (payout and refund failed). Any other ending succeeds normally. See "Quote destination test patterns" below for the full table.
</Note>
...
### Quote destination test patterns
...
Test error scenarios systematically using the magic account patterns. The
accounts below are the off-ramp quote's destination, so the quote suffixes
apply:

**1. Test quote execution failure (002):**

```bash
# Create account with the quote-execution-failure pattern
curl -X POST 'https://api.lightspark.com/grid/2025-10-13/customers/external-accounts' \
  -u "$GRID_CLIENT_ID:$GRID_CLIENT_SECRET" \
  -H 'Content-Type: application/json' \
  -d '{
...
# Execute an off-ramp quote to this account
curl -X POST 'https://api.lightspark.com/grid/2025-10-13/quotes/{quoteId}/execute' \
  -u "$GRID_CLIENT_ID:$GRID_CLIENT_SECRET"
# The transaction fails on the way to settlement with
# failureReason: QUOTE_EXECUTION_FAILED
```

**2. Test a long-running payment (003):**

```bash
# Create account with the long-payment pattern
curl -X POST 'https://api.lightspark.com/grid/2025-10-13/customers/external-accounts' \
  -d '{"customerId": "Customer:sandbox001", "accountNumber": "000000003", ...}'

# Execute a quote to this account - it stays PROCESSING for about
# six minutes before completing, so you can exercise pending-state handling
```
