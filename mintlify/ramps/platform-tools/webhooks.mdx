...
<Tab title="INTERNAL_ACCOUNT.STATUS_UPDATED">
  ...
  <Warning>
  Account status values: `PENDING` (provisioning), `ACTIVE` (ready for payments), `CLOSED` (customer-initiated), `FROZEN` (compliance/fraud hold), `FAILED` (provisioning failed). Frozen and failed accounts cannot send or receive payments.
  </Warning>
</Tab>
