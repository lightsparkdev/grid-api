...
| `CARD.FUNDING_SOURCE_CHANGE` | Every bind (`POST /cards/{id}/funding-sources`) and unbind (`DELETE /cards/{id}/funding-sources/{accountId}`). |
...
Fires on every bind and unbind. The `data` payload is the full `Card`
resource with the post-change `fundingSources` array, so a consumer
that only cares about the current set of bindings can replace state
wholesale.
