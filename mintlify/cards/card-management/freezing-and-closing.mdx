...
Freeze and close are sensitive state changes, so they use Grid's
...
| `ACTIVE` or `FROZEN` | `CLOSED` | `DELETE /cards/{id}` |
...
`DELETE /cards/{id}` is the only way to reach `CLOSED`, and the
