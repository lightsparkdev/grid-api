...
| `fundingSource.accountId` | Yes | The `InternalAccount` to bind. Must belong to the cardholder. |
...
## Adding more funding sources later

You can bind additional internal accounts to a card after issuance via
`POST /cards/{id}/funding-sources`. See
