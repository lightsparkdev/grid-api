...
A `CARD_TRANSACTION.UPDATED` webhook fires with the updated parent: the
...
Each row is the parent `CardTransaction` with child pull, clearing, and
refund events rolled up into the `pullSummary`, `settlementSummary`,
and `refundSummary` aggregates. See
[Listing transactions](/cards/transactions/listing-transactions) for
filtering, and [Reconciliation](/cards/transactions/reconciliation) for
