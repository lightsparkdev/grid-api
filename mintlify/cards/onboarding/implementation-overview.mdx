...
- Subscribe to the new card-specific webhook types (`CARD.STATE_CHANGE`,
  `CARD.FUNDING_SOURCE_CHANGE`, `CARD_TRANSACTION.UPDATED`).
...
- Build the `EXCEPTION` dashboard view (filter
  `GET /cards/{id}/transactions?status=EXCEPTION`) and wire it into
  on-call alerting.
