...
The external account's test pattern determines the outcome whenever that account is used,
whether you are pushing funds to it or pulling funds from it:
...
# Attempt to pull from it - should fail immediately
POST /quotes
...
# Attempt to push to it - should fail immediately
POST /quotes
...
- **Simplified KYC**: KYC processes are simulated rather than reviewed for real. On an unregulated platform the result resolves once you submit with `POST /verifications`; elsewhere it resolves at creation. You must add customers via the `/customers` endpoint, rather than using the KYC link flow.
