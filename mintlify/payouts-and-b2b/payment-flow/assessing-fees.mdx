...
- `CROSS_CURRENCY_TRANSACTION` — a transfer where the sending currency differs from the
  receiving currency.
- `RAIL` — a payout that goes out over a payment rail, whatever currency it started in.

The two stack. A cross-currency payout that goes out over ACH can carry both, and you are
paid for each.
