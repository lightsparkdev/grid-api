...
- `CROSS_CURRENCY_TRANSACTION` — a fee charged on transactions where the sending currency differs from the receiving currency
- `RAIL` — a fee charged on any transaction that uses this payment rail

Both fee types may apply to the same transaction, e.g. a cross-currency payment that goes out over ACH. Your platform will receive the sum of all applied fees.

<Info>
**Current limits.** Cross-currency fees are supported for a **USD** sending currency
only, and the fixed fee must be in that same currency — other currencies return
`NOT_IMPLEMENTED`.

Rail fees cover `ACH`, `ACH_SAME_DAY`, `RTP`, `FEDNOW` and `WIRE`, going `OUT`, with the fixed fee in USD.
`ACH` and `ACH_SAME_DAY` are separate rails, so price each one with its own config.
Anything else returns `NOT_IMPLEMENTED`.

Both are expanding — support for stablecoin sending currencies such as USDC is planned.
Watch the [changelog](/changelog) rather than assuming today's limits are permanent.
</Info>
...
`feeConfigs` is a **merge-by-key upsert**. Cross-currency fees are keyed by
`(feeType, sourceCurrency)`, rail fees by `(feeType, rail, direction, originator)`:
...
A rail fee looks like this:

```bash Set a standing rail fee
curl -X PATCH 'https://api.lightspark.com/grid/2025-10-13/config' \
  -u "$GRID_CLIENT_ID:$GRID_CLIENT_SECRET" \
  -H 'Content-Type: application/json' \
  -d '{
