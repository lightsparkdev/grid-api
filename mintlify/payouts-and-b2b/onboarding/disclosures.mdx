...
Record each acceptance with the `endUserTermsConsent` field when you create or update the customer — for both individual and business customers. Grid does not provision a customer's internal accounts until the acceptance is on file. See [Configuring Customers](/payouts-and-b2b/onboarding/configuring-customers#end-user-terms-consent) for the request shape and the fields to collect.
