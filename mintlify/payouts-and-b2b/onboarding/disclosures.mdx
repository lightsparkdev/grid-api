...
| `termsVersion` | Version identifier of the agreement the customer accepted, as returned for that `type` by `GET /customers/end-user-terms`. Send the version they actually accepted — Grid rejects a version it doesn't recognize for that type with `400 END_USER_TERMS_VERSION_NOT_FOUND`. |
