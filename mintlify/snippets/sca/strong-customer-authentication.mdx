...
Submit an `ScaAuthorization` proof to the authorization endpoint for the resource
that carries the challenge. Provide exactly one of `code` (for `SMS_OTP`) or
`passkeyAssertion` + `origin` (for `PASSKEY`):
...
labels each one (`CURRENCY_CONVERSION`, `PAYOUT`) for display only. A client
