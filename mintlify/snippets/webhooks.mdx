...
| `WALLET_OPERATION.*` | Retry with exponential backoff up to 7 days with maximum interval of 30 mins | No retry on 409 (duplicate webhooks). The terminal outcome can also be polled with `GET /operations/{operationId}` — see [Asynchronous operations](/api-reference/async-operations) |
