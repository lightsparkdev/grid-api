...
<Accordion title="Including purpose of payment">
  ...
  **For China business payouts:**
  ...
  Business payments to China must use one of the purposes below instead of `GOODS_OR_SERVICES`. Each one requires its own supporting documents. They also work on other corridors, where Grid sends the closest purpose the payout partner accepts.
