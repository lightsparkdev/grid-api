...
<Tip>
  The `paymentRail` field is optional. If omitted, Grid selects a default rail for the destination. Specify a rail (e.g., `ACH`, `ACH_SAME_DAY`, `WIRE`, `RTP`, `FEDNOW`) when you need to control which payment network processes the transfer. `ACH_SAME_DAY` is never selected for you: it requests same-business-day settlement and is priced separately. It is capped at $1,000,000 per entry by the NACHA same-day limit (rising to $10,000,000 on 2027-09-17); a payout above that is rejected, so send it over `ACH` instead.
</Tip>
...
<Accordion title="Including purpose of payment">
  ...
  **For China business payouts:**

  Business payments to China must use one of the purposes below instead of `GOODS_OR_SERVICES`. Each one requires its own supporting documents, listed in [Supporting Documents for China Business Payouts](/payouts-and-b2b/payment-flow/supporting-documents). They also work on other corridors, where Grid sends the closest purpose the payout partner accepts.

  A payment for services uses the purpose that names the service. `SERVICE_CHARGES` is not accepted on these payouts.

  - `EXPORTED_GOODS_PREPAYMENT` - Exported goods, paid before shipping
  - `EXPORTED_GOODS_POSTPAYMENT` - Exported goods, paid after delivery
  - `COMMISSION_ON_GOODS` - Commission on goods
  - `COMMISSION_ON_SERVICES` - Commission on services
  - `ACCOUNTING_SERVICES` - Accounting services
  - `EXHIBITION_SERVICES` - Exhibition services
  - `OFFICE_EXPENSES` - Office expenses
  - `DELIVERY_FEES` - Delivery fees
  - `HOTEL_ACCOMMODATION` - Hotel accommodation
  - `TRAVEL` - Travel, listed above and also valid for China
</Accordion>
</Step>
