...
<Accordion title="Including purpose of payment">
  ...
  **China business payouts only:**

  Business payments to China must use one of the purposes below instead of `GOODS_OR_SERVICES`. Each one requires its own supporting documents. These codes are rejected on other corridors.
