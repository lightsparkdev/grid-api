...
Children are reconciled against the parent and rolled up into its
`settledAmount` and `refundedAmount`. You don't see per-child rows on
the list endpoint — only the running totals on the parent.
...
`settledAmount: 1500` — a `settledAmount` above the hold is how you
recognize the over-auth.
