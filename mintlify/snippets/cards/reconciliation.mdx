...
AUTH REQUEST ┤
             └──► AUTHORIZED ──► PARTIALLY_SETTLED ──► SETTLED
                      ├──► VOIDED     (full reversal or expiry, no clearing)
...
| `VOIDED` | The authorization was fully reversed or expired before any clearing posted. The hold closed without moving money, `authorizedAmount` is 0, and `settledAmount` is absent. |
