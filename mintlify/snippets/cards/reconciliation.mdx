...
```text
AUTH REQUEST ──┬──► DECLINED
               └──► AUTHORIZED ──┬──► VOIDED
                                 └──► CLEARING ──┬──► PARTIALLY_SETTLED ──► SETTLED
                                                └──► EXCEPTION
```
...
| `VOIDED` | The authorization was fully reversed or expired before any clearing posted. The hold closed without moving money, `authorizedAmount` reports what is still held (normally 0), and `settledAmount` is absent. |
