...
Every update is delivered via a `CARD.TRANSACTION_UPDATE` webhook
carrying the post-update parent. See
[Webhooks](/cards/platform-tools/webhooks) for the full payload shape.
...
Signal to watch: a `CARD.TRANSACTION_UPDATE` webhook with
`data.status: "EXCEPTION"`. The payload includes the full parent record,
so your dashboard's exception view is driven entirely by webhook
deliveries — there's no list endpoint to poll.
...
Every `CARD.TRANSACTION_UPDATE` webhook carries a unique `id`. Track
