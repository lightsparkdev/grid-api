...
provisions it. Grid assigns `platformCardId` itself today, so read the
identifier back from the returned `Card` rather than relying on a value you
supplied. When activation completes, a
