...
Set `maxSpendPerTransaction` and `maxSpendPerDay`. Spend controls work the same
way for a wallet-funded card as for any other card: the values on the card cap
what that card can authorize, and `cardConfigs` in your platform config can set
platform-wide caps that apply to every card you issue. When both are set, Grid
