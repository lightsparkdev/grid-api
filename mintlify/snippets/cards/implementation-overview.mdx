...
- Subscribe to the card webhook types: `CARD.STATUS_CHANGE` for card
  state, `CARD.PIN_STATUS_CHANGE` for PIN status, and `CARD_TRANSACTION.AUTHORIZED`,
  `CARD_TRANSACTION.PARTIALLY_SETTLED`, `CARD_TRANSACTION.SETTLED`,
  `CARD_TRANSACTION.DECLINED`, `CARD_TRANSACTION.VOIDED`, and
...
- If your program includes physical cards, start the artwork early:
  design review, Visa approval, and manufacturing run for weeks. See
  [Card design guidelines](/cards/card-design/guidelines).
...
Every card is bound to one `InternalAccount` as its funding source at
...
- See [Funding sources](/cards/card-management/funding-sources) for
  rules around binding and replacing the source.
- A card funded from a customer's Embedded Wallet also needs a delegated
  signing key, created once by the cardholder, before authorizations can
  pull from it. See
  [Embedded Wallet funding](/cards/card-management/embedded-wallet-funding).
...
`CARD.STATUS_CHANGE` webhook. Day-to-day operational changes are:
...
Cardholders can add a card to Apple Pay, Google Pay, or Samsung Pay by
typing its details into the wallet with no work on your side. An
in-app "Add to Wallet" button needs wallet-provider approval plus one
`POST /cards/{id}/tokenize` call from your backend. See
[Digital wallet tokenization](/cards/card-management/digital-wallet-tokenization).

## Transactions and reconciliation

Each authorization on a card produces a `CardTransaction` row, with
...
refused before approval, `VOIDED` for approved auths fully reversed or
expired before clearing, and `EXCEPTION` as the failure path for stuck
post-hoc pulls. A merchant return is its own dated `CREDIT`
