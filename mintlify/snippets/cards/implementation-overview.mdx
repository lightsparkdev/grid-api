...
high-urgency path when the network settled but the funding-source pull
failed. A declined authorization starts and ends in `DECLINED`, with no
funds pulled. A merchant return keeps the
