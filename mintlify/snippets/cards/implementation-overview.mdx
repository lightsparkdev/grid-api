...
- Subscribe to the new card-specific webhook types (`CARD.STATE_CHANGE`,
  `CARD.FUNDING_SOURCE_CHANGE`, and `CARD.TRANSACTION_UPDATE`).
...
- Build the `EXCEPTION` dashboard view from
  `CARD.TRANSACTION_UPDATE` webhooks (filter by
  `data.status: "EXCEPTION"`) and wire it into on-call alerting.
