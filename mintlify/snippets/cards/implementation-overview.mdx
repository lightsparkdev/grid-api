...
- Subscribe to the card webhook types: `CARD.STATE_CHANGE` for card
  ...
  `CARD_TRANSACTION.DECLINED` for the transaction lifecycle.
...
`AUTHORIZED → PARTIALLY_SETTLED → SETTLED`, with `DECLINED` as the
failure path for refused auths and stuck post-hoc pulls. A merchant
return keeps the transaction `SETTLED` and reports the returned value
in `refundedAmount`.
...
- Build the `DECLINED` dashboard view from `CARD_TRANSACTION.DECLINED`
