...
issue time. Authorization Decisioning checks the source's balance before
approving each auth, so:
...
- See [Funding source](/cards/card-management/funding-sources) for the
  rules around binding and rebinding.
