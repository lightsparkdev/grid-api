...
Every card must be bound to one `InternalAccount` at issue time. The
account must:
...
the card network, and the pull against the funding source fails afterwards.
The transaction resolves as `EXCEPTION` for you to reconcile, and it carries
