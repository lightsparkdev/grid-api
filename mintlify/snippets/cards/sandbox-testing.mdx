...
Authorization outcomes are deterministic and driven by magic-value suffixes
on `merchant.descriptor`. Other Sandbox helpers use the identifier suffixes
...
| Suffix | Outcome |
|--------|---------|
| `002` | Declined, `cardDeclinedReason: INSUFFICIENT_FUNDS`. Sandbox only: production approves an underfunded authorization and resolves it as `EXCEPTION` when the pull fails |
| `003` | Declined, `cardDeclinedReason: CARD_NOT_ACTIVE`. Use against a `FROZEN` card to verify it refuses authorizations |
...
- `amount = 0` exercises `AUTHORIZATION_EXPIRY` — the auth expires
  with no clearing posted, moves to `VOIDED`, and sends
  `CARD_TRANSACTION.VOIDED`.
...
A refund posts as its own dated `CREDIT` `CardTransaction` linked to the
...
At each step you'll see `CARD.STATUS_CHANGE` webhooks plus transaction
