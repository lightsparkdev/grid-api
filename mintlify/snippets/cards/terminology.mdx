...
The `InternalAccount` bound to a card. Every card has one funding source,
and Authorization Decisioning checks its balance at auth time before
approving a transaction. The `fundingSource` field on the `Card` resource
...
or declines it, creating a terminal card transaction with
`status: "DECLINED"` and a `cardDeclinedReason`.
...
A full authorization reversal or an authorization expiry before any
clearing closes the hold without moving money. The card transaction is
terminal with `status: "VOIDED"`. `authorizedAmount` reports what is
still held, normally 0, and `settledAmount` is absent.

## Refund

A merchant-initiated `RETURN` against a settled transaction. Refunds
flow back to the funding source. A refund against a known purchase is its
own dated `CardTransaction` row (`direction: CREDIT`) linked to the
...
A short-lived credential minted by `POST /cards/{id}/reveal` whose
