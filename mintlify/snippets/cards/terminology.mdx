...
The `InternalAccount` bound to a card. Every card has exactly one funding
source, named by the `fundingSource` field on the `Card` resource, and
Authorization Decisioning checks its balance at auth time before approving
a transaction.
