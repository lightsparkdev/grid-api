...
A transaction the issuer refused, or one that settled to the card
network but whose corresponding pull from the funding source failed —
for example, the cardholder's balance no longer covers a post-hoc tip
clearing. Declines are the high-urgency reconciliation alerts and are
surfaced as `status: "DECLINED"` on the parent `CardTransaction`.
