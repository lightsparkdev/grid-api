...
No webhook fires for a funding source change. The `200` response to the
signed retry carries the post-change `Card` resource, and `GET /cards/{id}`
returns the current list at any time.
