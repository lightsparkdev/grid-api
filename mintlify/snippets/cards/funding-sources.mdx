...
You cannot supply `fundingSource` alongside `state: CLOSED`. To stop a card
from spending, set `state: FROZEN` instead.
...
| 400 | `INVALID_INPUT` | `fundingSource` was supplied alongside `state: CLOSED`, or another request field is invalid. |
