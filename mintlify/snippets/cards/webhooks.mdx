...
| Type | Fires on |
|------|----------|
| `CARD.STATE_CHANGE` | `PROCESSING → ACTIVE`, `PROCESSING → PENDING_AUTH`, `PENDING_AUTH → ACTIVE`, `→ CLOSED (ISSUER_REJECTED)`, and every subsequent `ACTIVE ⇄ FROZEN` and `→ CLOSED` transition. |
