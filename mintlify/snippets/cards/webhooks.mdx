...
- `state: "ACTIVE"` after `PROCESSING` — the card is live. To reveal
  the full card details, create a reveal session with
  `POST /cards/{id}/reveal-session` right before rendering its
  short-lived `panEmbedUrl` in an iframe — webhook payloads never carry
  a reveal URL.
