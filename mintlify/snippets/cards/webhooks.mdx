...
| `CARD.TRANSACTION_UPDATE` | Whenever a `CardTransaction` is created or updated by authorization, clearing, refund, or reconciliation activity. |
...
## CARD.TRANSACTION_UPDATE

Fires whenever a `CardTransaction` is created or updated by
authorization, clearing, refund, or reconciliation activity. The
`data` payload is the full post-update `CardTransaction`, including
its lifecycle status and reconciliation aggregates.

```json
{
  "id": "Webhook:019542f5-b3e7-1d02-0000-000000000040",
  "type": "CARD.TRANSACTION_UPDATE",
