...
To show the cardholder their full PAN, CVV, and expiry, create a reveal
session right before rendering:

```bash
curl -X POST "$GRID_BASE_URL/cards/Card:019542f5-b3e7-1d02-0000-000000000010/reveal-session" \
  -u "$GRID_CLIENT_ID:$GRID_CLIENT_SECRET"
```
...
<Note>
The reveal session is the **only** way to obtain a reveal URL — the
`Card` resource never carries one (not in responses, not in webhook
payloads). The URL expires at `expiresAt` (within minutes), so create a
session each time the cardholder asks to reveal their details,
immediately before rendering the iframe. Never store, cache, or log the
URL. Every session is audit-logged.
</Note>
