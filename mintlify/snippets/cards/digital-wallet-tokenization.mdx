...
4. In the app, start the add-to-Wallet flow through the TapAndPay SDK.
   Under Google's Unified Push Provisioning (UPP), Google Wallet runs the
   screens itself and then calls your app back asking for the card's
   payment credentials, handing you a `serverSessionId` that identifies
   this session. Send it, with the wallet account id, to Grid with
   `POST /cards/{id}/tokenize`, and return Grid's payload from that
   callback.
