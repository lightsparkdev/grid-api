...
from your backend. This page covers both.

## Manual entry: nothing to build

A cardholder opens their wallet app, chooses to add a card, and types in
the card number, expiry, and CVV (or scans the card). The wallet asks
...
your app with one tap, without typing anything. The wallet SDK on the
device produces a set of values, your backend exchanges them with Grid
for an encrypted payload, and the SDK completes the add. The payload is
encrypted to the wallet provider's keys, so neither your app nor your
servers ever see card data.
...
sequenceDiagram
  ...
  A->>B: Relay values
  B->>G: POST /cards/{id}/tokenize { wallet, … }
  G-->>B: 200 { encrypted provisioning payload }
  B-->>A: Relay payload
...
the approvals are the long pole — start them early.
...
4. In the app, present `PKAddPaymentPassViewController`. Apple calls
   your delegate with `certificates`, `nonce`, and `nonceSignature`;
   relay them to your backend and call Grid.

### Google Pay

1. Request access to Google's
   [Push Provisioning API](https://developers.google.com/pay/issuers/apis/push-provisioning/android)
   through Google's support form.
2. Submit your "Add to Google Wallet" flow for Google's UX review.
3. Google approves and allowlists your app. Integrate the TapAndPay SDK.
4. Under Google's Unified Push Provisioning (UPP), the SDK starts the
   flow and calls your app back for payment credentials with a
   `serverSessionId`. Relay it to your backend and call Grid inside
   that callback.

<Warning>
Google is ending support for its legacy in-app push provisioning at the
end of 2026. Grid supports UPP only, so new integrations must use the
UPP credentials callback.
</Warning>
...
4. The SDK reports the wallet's `walletUserId` and `deviceId`; relay
   them to your backend and call Grid.

## The API call

`POST /cards/{id}/tokenize` takes the wallet name plus the values the
SDK produced and returns the payload the SDK needs. Call it from your
backend with your platform credentials — never from the app — and hand
the response straight back to the SDK. Do not store or log the payload;
it is valid for one provisioning attempt.
