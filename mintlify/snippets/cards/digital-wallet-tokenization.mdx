...
ships a library your app integrates to start that flow; this page calls
it the wallet SDK:
...
them to your backend, your backend exchanges them with Grid for an
encrypted payload, and the wallet SDK uses that payload to finish adding
the card. The payload is encrypted to the wallet provider's keys, so
neither your app nor your servers ever see card data.
