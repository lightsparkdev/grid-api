...
The request and response bodies are discriminated by `wallet`: each
...
| `wallet` | Request shape | Response shape |
|----------|---------------|----------------|
| `APPLE_PAY` | `certificate`, `nonce`, `nonceSignature` (all required) | `applePay` object |
| `GOOGLE_PAY` | `walletAccountId`, `serverSessionId` (required); `deviceId`, `opaquePaymentCardRequested` (optional) | `provisioningPayload`, plus `googleAccountPayload` when requested |
| `SAMSUNG_PAY` | `walletAccountId`, `deviceId` (both required) | `provisioningPayload` |
...
| Status | Code | When |
|--------|------|------|
| `400` | `INVALID_INPUT` | The body does not match the request shape for its `wallet` (a required field is missing, or a field from another wallet's shape was supplied), or the wallet provider rejected the values (for example, an expired Apple nonce). Restart the add-to-wallet flow in the app to get fresh values. |
| `401` | `UNAUTHORIZED` | Your platform credentials were rejected, or your production platform is not yet enabled for card issuing. Contact your Lightspark representative for the latter. |
| `403` | `FORBIDDEN` | The credential lacks `uma:manage`, or the session has no attributable actor to audit the request against (for example, an impersonated dashboard session). Retry with a platform API credential. |
...
| `500` | `INTERNAL_ERROR` | The card issuer could not be reached or returned an unusable response. Retry; if it persists, contact support. |
