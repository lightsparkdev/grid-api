...
| Digital wallet art (Apple Pay, Google Pay) | Your app icon and logo, plus two colors | Card image, derived from your front; Visa DEBIT lockup placement |
| Carrier and envelope | Nothing | Standard carrier letter and unbranded envelope |
...
The two bodies on this page are what most programs print. The manufacturer
also offers premium options, each of which raises the per-card cost, adds a
minimum order, and adds weeks to months of lead time:

| Option | What it is | What it changes for your design |
| --- | --- | --- |
| Metal | Stainless steel core with laminated faces, or full metal | Logo is etched, not printed, so it has no color. Needs Visa's approval of the material. The longest lead time. |
| Colored core | Plastic tinted through the card | The edge and any unprinted area take the core color. |
| Spot gloss | Clear high-gloss varnish over the logo on a matte card | One-color effect from a vector logo; reads by sheen, not ink. |
| Foil logo | Hot-stamped metallic foil, as the Visa lockup is | One color, from a vector logo. The only way to get a metallic look; metallic inks do not print in CMYK. |
| Colored magnetic stripe | Stripe in a color other than black | Cosmetic; the stripe still reads in every terminal. |
| Translucent or recycled plastic | Clear, tinted, or recycled PVC body | Translucent bodies change how every color reads; recycled stock can carry a printed statement saying so. |

Decide on these before you design, because most of them change what the
artwork can be. Tell your Lightspark contact what you want, and we scope cost
and schedule with the manufacturer.

The same goes for a vertical orientation or a custom back layout: both are
possible, both go through the same review, and both need a conversation
before you start.
