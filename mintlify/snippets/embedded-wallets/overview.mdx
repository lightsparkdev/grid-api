...
<Note>
  **Testing in sandbox.** Sandbox accepts two fixed magic values so you can exercise the auth + signing flow without managing real OTP delivery or device-side keys:
  ...
  - **Wallet signature:** the `Grid-Wallet-Signature` header accepts the literal string `sandbox-valid-signature` for any signed wallet action. The `encryptedSessionSigningKey` returned by `/verify` in sandbox is a stub and is not meant to decrypt — skip the HPKE decrypt step entirely.
...
You will receive an `INTERNAL_ACCOUNT.BALANCE_UPDATED` webhook when the balance updates. The wallet now holds 1,000.00 USDB.
