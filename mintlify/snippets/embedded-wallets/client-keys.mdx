...
<Note>
  **In sandbox, send `Grid-Wallet-Signature: sandbox-valid-signature`** for any signed wallet action. Sandbox skips the ECDSA check, so you don't need a real session signing key or an extracted `payloadToSign`. The signing pattern below applies only to production.
</Note>
