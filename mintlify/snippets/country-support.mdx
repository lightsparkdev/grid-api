...
<FeatureCardContainer>
  <FeatureCardList>
    ...
    <FeatureCard icon="/images/icons/blocks.svg" title="Full Platform Access" tag="United States & Europe" tagPosition="inline" layout="horizontal" variant="flat">
      ...
      Send payments to 64 countries on local banking rails, in addition to BTC and stablecoins.
    </FeatureCard>
    <FeatureCard icon="/images/icons/globe.svg" title="Local Payment Rails" tag="64 Countries" tagPosition="inline" layout="horizontal" variant="flat">
...
| 🇺🇸 United States | US | `ACH` `Same-day ACH` `Wire Transfer` `RTP` `FedNow` `SWIFT` |
...
<FeatureCardGrid cols={4}>
  ...
  <FeatureCard icon="/images/icons/globe.svg" title="Middle East and Africa" tag="17 countries">
...
<Note>
China already has live coverage in the table above, via Thunes (`AliPay` `WeChat Pay` `Bank Transfer`).
The entry here is a separate, in-progress corridor through a different provider. It doesn't reduce or
replace the existing China coverage.
</Note>
