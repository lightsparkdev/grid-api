...
<Steps>
  ...
  <Step title="Your backend calls Grid">
    ...
    Grid verifies the attestation and replies `201` with the new `AuthMethod:...` id. Persist the auth method id against the customer — to authenticate later you'll call `POST /auth/credentials/{id}/challenge` followed by `/verify`.
  </Step>
</Steps>
