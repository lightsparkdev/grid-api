...
<Steps>
  ...
  <Step title="Client stamps the payload">
    Build a Grid wallet signature over `payloadToSign` with the current session signing key.
  </Step>
...
<Steps>
  ...
  <Step title="Client stamps the payload">
    Build a Grid wallet signature over `payloadToSign` with any active session signing key on the same account — either the session being revoked (self-logout) or another session (admin-style sign-out of a different device).
  </Step>
