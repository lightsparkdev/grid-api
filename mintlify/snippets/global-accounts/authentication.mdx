...
A single internal account can hold one `ACTIVE` `EMAIL_OTP` credential, one `ACTIVE` `SMS_OTP` credential, and multiple distinct `PASSKEY` credentials concurrently. `OAUTH` credentials can be added for each supported provider identity.
...
The client generates a fresh P-256 key pair (the TEK — Target Encryption Key), HPKE-encrypts `{otp_code, public_key}` under `otpEncryptionTargetBundle`, and submits the encrypted payload. `public_key` must be the compressed public key (`02` or `03` prefix). See <a href="client-keys#encrypt-the-otp-code-email_otp-only">Encrypt the OTP code</a> for implementation details.
...
### Changing the email or phone number

The `EMAIL_OTP` address comes from the customer email on file, and the `SMS_OTP` number from the customer phone number. Changing either one changes how the customer signs in, so Grid applies the change only after the customer proves they control the new contact. The change then needs a signature from their wallet session.

<Warning>
  Breaking change. `PATCH /customers/{customerId}` no longer changes the email or phone number of a customer with tied Embedded Wallet OTP credentials until the new contact is verified. Without a verified credential it returns `400` `AUTH_CREDENTIAL_VERIFICATION_REQUIRED`. A customer without tied OTP credentials is unaffected.
</Warning>

<Steps>
  <Step title="Register the new contact">
    Create an OTP credential carrying the new contact: `email` for `EMAIL_OTP`, `phoneNumber` for `SMS_OTP`. Any of the customer's Global Account ids works as `accountId`. There is no signed retry. Grid sends a one-time password to the new contact and returns a `PENDING` credential.
    ...
    **Response (201):**
    ...
    ```json
    {
      "id": "AuthMethod:019542f5-b3e7-1d02-0000-000000000005",
      "accountId": "InternalAccount:019542f5-b3e7-1d02-0000-000000000002",
      "type": "EMAIL_OTP",
      "status": "PENDING",
      "nickname": "jane.new@example.com",
      "otpEncryptionTargetBundle": "{\"version\":\"v1.0.0\",\"data\":\"7b227461...\",\"dataSignature\":\"30450221...\",\"enclaveQuorumPublic\":\"04a1b2c3...\"}",
      "createdAt": "2026-04-10T09:00:00Z",
      "updatedAt": "2026-04-10T09:00:00Z"
    }
    ```

    A `PENDING` credential cannot be used to sign in. To resend the code, call `POST /auth/credentials/{id}/challenge`.

    This path needs an existing `ACTIVE` credential of the same type. If the customer has none, a new `email` or `phoneNumber` is rejected with `400` `INVALID_INPUT`; change the contact with `PATCH /customers/{customerId}` alone.
  </Step>
  <Step title="Verify the new contact">
    Build the `encryptedOtpBundle` from the code the customer received, exactly as for a login, and call `verify`. On a `PENDING` credential, verify completes in one call. It returns `200` with the credential in `status: VERIFIED`. There is no `202` signed step and no session.

    ```bash
    curl -X POST "$GRID_BASE_URL/auth/credentials/AuthMethod:019542f5-b3e7-1d02-0000-000000000005/verify" \
      -u "$GRID_CLIENT_ID:$GRID_CLIENT_SECRET" \
      -H "Content-Type: application/json" \
      -d '{
        "type": "EMAIL_OTP",
    ...
    Grid keeps the proof server-side. It expires 15 minutes after verification. After that, call `challenge` and `verify` again.
  </Step>
  <Step title="Update the customer">
    Call `PATCH /customers/{customerId}` with the new value. Grid finds the newest `VERIFIED` credential for that value and returns `202` with `payloadToSign` and `requestId`. The client stamps `payloadToSign` with the session signing key of an `ACTIVE` credential, and you retry the same request with `Grid-Wallet-Signature` and `Request-Id`. This is the usual <a href="#the-signed-retry-pattern">signed-retry pattern</a>.
    ...
    The signed retry returns `200` with the updated customer. It can also return a <a href="#handling-a-still-processing-response">still-processing `200`</a>. When the change finishes, Grid sends a `WALLET_OPERATION.COMPLETED` or `WALLET_OPERATION.FAILED` webhook with `operationType: customer.contact_update`, the customer id as `resourceId`, and your `Request-Id` as `requestId`. You can also re-send the identical signed retry until you get the customer.
  </Step>
</Steps>

When the change completes, the new credential becomes `ACTIVE` and replaces the customer's previous `ACTIVE` credential of that type, and the customer's `email` or `phoneNumber` changes with it. All of the customer's Global Accounts share one wallet, so the change covers every one of them. Sessions issued from the old credential keep working until they expire. Grid updates the customer's records with the card issuer and banking partner shortly afterwards.

| Error | Meaning |
|---|---|
| `400` `AUTH_CREDENTIAL_VERIFICATION_REQUIRED` | No `VERIFIED` credential exists for the new contact. Register and verify it first. |
| `400` `AUTH_CREDENTIAL_VERIFICATION_EXPIRED` | The verification expired. Call `challenge`, then `verify` again. |
| `409` `CONTACT_UPDATE_IN_PROGRESS` | Another update to this contact hasn't finished. Retry later. |

To cancel a change, delete the `PENDING` or `VERIFIED` credential with `DELETE /auth/credentials/{id}`. It is a single call with no signed step and returns `204`. The sign-in contact stays as it was. `GET /auth/credentials` lists unfinished changes alongside `ACTIVE` credentials; check each credential's `status`.
...
Adding an additional credential, revoking a credential, refreshing or revoking a session, exporting a wallet, updating wallet privacy, and changing the email or phone number of a customer with tied OTP credentials all share the same shape:
...
| `PATCH /customers/{id}` | Change a customer's email or phone number |
...
<Note>
  `verify`, `challenge`, `export`, and `PATCH /customers/{id}` already use `200` for their terminal success body (`AuthSession`, the challenge response, the encrypted credentials, or the customer). For those four, `200` alone doesn't tell you which case you're in — check the response body's `status` field for `"PROCESSING"` to distinguish it from a settled success. The other four endpoints (`add credential`, `session refresh`, `revoke credential`, `revoke session`) use `201` or `204` on success, so a `200` by itself already means still-processing.
</Note>
