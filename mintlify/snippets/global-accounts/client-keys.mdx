...
`EMAIL_OTP` credentials never send the OTP code in plaintext. Instead, the client HPKE-encrypts the code (together with the compressed form of its public key) to an enclave key, so the code is unreadable in transit and Grid is only a pass-through.
...
Set `public_key` to the **compressed** SEC1 public key: a 66-character hex string starting with `02` or `03`. The step 1 samples export the uncompressed form (130 characters, `04` prefix), so compress it first. If the bundle carries an uncompressed key, the enclave rejects the OTP code and the first verify call returns:
...
function buildEncryptedOtpBundle(
  ...
  // The enclave expects snake_case { otp_code, public_key } and a compressed
  // public key (66 hex characters, 02 or 03 prefix).
