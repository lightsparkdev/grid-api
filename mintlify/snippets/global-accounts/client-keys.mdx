...
Generate a fresh P-256 key pair for every authentication, session refresh, and wallet export. Where the public key goes depends on the flow:

- `PASSKEY`: send it as `clientPublicKey` on `POST /auth/credentials/{id}/challenge`.
- `OAUTH`: send it as `clientPublicKey` on `POST /auth/credentials/{id}/verify`.
- `EMAIL_OTP`: put it inside `encryptedOtpBundle` on `POST /auth/credentials/{id}/verify`. See <a href="#encrypt-the-otp-code-email_otp-only">Encrypt the OTP code</a>.
- Session refresh: send it as `clientPublicKey` on both `/auth/sessions/{id}/refresh` calls.
- Wallet export: send it as `clientPublicKey` on both `/export` calls.

Keep the private key in device-local secure storage (browser `IndexedDB` gated by Web Crypto's non-extractable flag, iOS Keychain, Android Keystore). Send the public key hex-encoded through your integrator backend. For the recommended client-held-key flow, send the **compressed** form — a 66-character string starting with `02` or `03` (see [Client-held session key](#client-held-session-key)). For the legacy flow, send the **uncompressed** form — a 130-character string starting with `04`, which the Web Crypto, iOS, and Android APIs below produce natively.
...
`EMAIL_OTP` credentials never send the OTP code in plaintext. Instead, the client HPKE-encrypts the code, together with its public key, to an enclave key, so the code is unreadable in transit and Grid is only a pass-through.
...
`public_key` must be the **compressed** public key: 66 hex characters starting with `02` or `03`. The step 1 samples produce the uncompressed form (130 hex characters starting with `04`), so compress the key before you encrypt it. The example below does this for you. The `verificationToken` Grid returns is bound to this exact key, so the same compressed hex must be the `publicKey` in the `Grid-Wallet-Signature` stamp on the signed retry (see <a href="#4-sign-a-payloadtosign">step 4</a>); a different value there fails the login.

In production, an uncompressed key makes the enclave reject the OTP code, even when the code is correct. The first verify call then returns:

```json
{"code": "INVALID_INPUT", "reason": "Invalid wallet request."}
```

Grid returns this same response for other rejected requests too, including a mistyped OTP code. If you get it with a code you know is correct, check the key format first.
...
declare function hpkeEncryptToGridBundle(args: {
...
// clientPublicKeyHex: the public key from step 1; this function compresses it before encrypting
...
function buildEncryptedOtpBundle(
  ...
  // The enclave expects snake_case keys and a compressed public key.
  const compressedPublicKeyHex = p256.Point.fromHex(clientPublicKeyHex).toHex(true);
  const plainTextBuf = new TextEncoder().encode(
    JSON.stringify({ otp_code: otp, public_key: compressedPublicKeyHex }),
  );
  return hpkeEncryptToGridBundle({
    plainTextBuf,
    targetKeyBuf: hexToBytes(targetPublic),
  });
}
```
...
The wire format is a base58check string. Decoded, the payload is a 33-byte compressed P-256 encapsulated public key followed by AES-256-GCM ciphertext (ciphertext || 16-byte auth tag). For HPKE itself, uncompress the encapsulated key to 65-byte SEC1 form, use Grid's session-bundle HPKE info value, and authenticate with AAD `encappedPublicUncompressed || recipientPublicKeyUncompressed`.
...
declare function gridSessionBundleHpkeInfo(): Uint8Array;
...
async function decryptSessionSigningKey(
  ...
  const recipient = await suite.createRecipientContext({
    ...
    enc,
    info: gridSessionBundleHpkeInfo(),
  });
...
fun decryptSessionSigningKey(
    ...
    val recipient = hpke.setupBaseR(enc, privateKey, gridSessionBundleHpkeInfo())
...
- `DELETE /auth/credentials/{id}`, `DELETE /auth/sessions/{id}`, `POST /auth/sessions/{id}/refresh`, `POST /internal-accounts/{id}/export`, `PATCH /internal-accounts/{id}`, `PATCH /customers/{id}` for email or phone number changes on customers with tied OTP credentials — all 202 response bodies.

Stamp the payload **byte-for-byte as returned** (do not re-parse, re-serialize, or trim whitespace). The session signing key produces a Grid wallet signature: derive its compressed P-256 public key, sign the payload with the private scalar, then base64url-encode the signature envelope:
...
<CodeGroup>
```typescript Web (TypeScript)
declare function deriveCompressedP256PublicKey(privateKeyHex: string): string;
declare function signP256Sha256Der(args: {
  content: string;
  privateKeyHex: string;
}): Promise<string>;

const GRID_STAMP_SCHEME = "SIGNATURE_SCHEME_TK_API_P256";
...
async function buildGridWalletSignature(
  ...
  const apiPublicKey = deriveCompressedP256PublicKey(apiPrivateKey);
  const signature = await signP256Sha256Der({
    content: payloadToSign,
    privateKeyHex: apiPrivateKey,
  });
  const stamp = JSON.stringify({
    publicKey: apiPublicKey,
    scheme: GRID_STAMP_SCHEME,
    signature,
  });
...
private const val GRID_STAMP_SCHEME = "SIGNATURE_SCHEME_TK_API_P256"
...
fun buildGridWalletSignature(
    ...
    val stampJson = """{"publicKey":"$publicKeyHex","scheme":"$GRID_STAMP_SCHEME","signature":"$derSignatureHex"}"""
