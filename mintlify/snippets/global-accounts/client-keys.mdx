...
`public_key` must be the **compressed** public key: 66 hex characters starting with `02` or `03`. The step 1 samples produce the uncompressed form (130 hex characters starting with `04`), so compress the key before you encrypt it. The example below does this for you.
