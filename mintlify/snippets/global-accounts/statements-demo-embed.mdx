export const StatementsDemoEmbed = () => {
  React.useEffect(() => {
    ...
    const base = isLocal
      ? 'http://localhost:4003'
      : branch
        ? 'https://grid-statements-demo-git-' + branch + '-lightspark-team.vercel.app'
    ...
    if (!host) {
      ...
      iframe.title = 'Grid Statements — playground';
      iframe.setAttribute('allow', 'clipboard-write');
      iframe.style.cssText = 'width:100%;height:100%;border:0;display:block;';
      host.appendChild(iframe);
      document.body.appendChild(host);
    }
    if (window.__sdHideTimer) { clearTimeout(window.__sdHideTimer); window.__sdHideTimer = null; }
    ...
    const sync = () => {
      const el = document.getElementById('statements-demo-container');
      if (el && host) {
    ...
    return () => {
      ...
      window.__sdHideTimer = setTimeout(() => {
        const h = document.getElementById('statements-demo-host');
