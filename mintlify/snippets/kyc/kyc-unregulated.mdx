...
<Step title="Track the decision">
  ...
  - **Webhook (recommended):** Subscribe to `CUSTOMER.KYC_APPROVED` / `CUSTOMER.KYC_REJECTED` (and `CUSTOMER.KYB_APPROVED` / `CUSTOMER.KYB_REJECTED` for business customers) to be notified when the customer reaches a terminal status (`APPROVED` or `REJECTED`).
...
<Step title="Track the decision">
  Track terminal `kycStatus` transitions via the `CUSTOMER.KYC_APPROVED` / `CUSTOMER.KYC_REJECTED` webhook (recommended) or by polling `GET /customers/{customerId}`. On `APPROVED`, unlock funding and money movement.
</Step>
</Steps>
</Tab>
...
<Step title="Track the decision">
  Track terminal `kycStatus` transitions via the `CUSTOMER.KYC_APPROVED` / `CUSTOMER.KYC_REJECTED` webhook (recommended; use the `CUSTOMER.KYB_*` siblings for business customers) or by polling `GET /customers/{customerId}`. On `APPROVED`, unlock funding and money movement.
</Step>
</Steps>
</Tab>
</Tabs>
