...
<Step title="Verify email and phone (only where required)">
  Some regions (for example, the EU) require the customer's email and phone number to be verified before KYC can begin. When this applies, the customer carries a `contactVerification` object (e.g. `{ "email": "PENDING", "phone": "PENDING" }`); when it's absent, no contact verification is required and you can skip this step.

  Drive each channel with a send-then-confirm pair (the provider delivers a one-time code by email/SMS):
  ...
  Re-POST the send endpoint to resend a code. Each channel moves `PENDING` → `VERIFIED`.

  <Note>
  Generating the KYC link before both channels are `VERIFIED` returns `409`.
  </Note>
</Step>
