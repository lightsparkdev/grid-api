export const FeatureCard = ({ icon, title, children, href, linkHref, linkText, color, tag, layout }) => {
  const isHorizontal = layout === 'horizontal';
  const card = (
    <div className={`feature-card ${href ? 'feature-card-link' : ''} ${!icon ? 'feature-card-no-icon' : ''} ${isHorizontal ? 'feature-card-horizontal' : ''}`}>
      {icon && (
