...
**Solution:** Read `details.minimumAmount` and `details.maximumAmount` and retry inside the band. Each is a `CurrencyAmount` in the currency the bound applies to. A bound is omitted when the rejecting step does not know it.
