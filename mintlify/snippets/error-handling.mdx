...
| `408 Request Timeout`       | Request timed out       | The request body could not be read in time; retry       |
...
<Accordion title="AMOUNT_OUT_OF_RANGE">
**Cause:** The amount is below the minimum or above the maximum for this payment. A common case is a sending amount too small to cover the payout provider's fee.

**Solution:** Retry with an amount between `details.minimumAmount` and `details.maximumAmount`. Each is a `CurrencyAmount`. Check its `currency`: the limit can be in the sending or the receiving currency. A limit is omitted when it is not known.

```json
{
  "code": "AMOUNT_OUT_OF_RANGE",
  "reason": "The sending amount is too small to cover the payout provider's fee.",
  "details": {
    "minimumAmount": { "amount": 27, "currency": { "code": "USD" } }
  }
}
```

```javascript
async function createQuoteInsideBand(params) {
  try {
    return await createQuote(params);
  } catch (error) {
    const min = error.details?.minimumAmount;
    if (error.code === "AMOUNT_OUT_OF_RANGE" && min?.currency.code === params.currency) {
      return await createQuote({ ...params, amount: min.amount });
    }
    throw error;
  }
}
...
<Accordion title="TRANSACTION_SIZE_LIMIT_EXCEEDED">
**Cause:** The requested amount exceeds the maximum single-transaction limit for the trade corridor or withdrawal currency

**Solution:** Reduce the transaction amount or split into multiple smaller transactions

```javascript
async function handleLargeTransfer(amount, maxPerTransaction) {
  if (amount <= maxPerTransaction) {
    return await createQuote({ amount });
  }
  ...
  while (remaining > 0) {
    const batchAmount = Math.min(remaining, maxPerTransaction);
    batches.push(batchAmount);
    remaining -= batchAmount;
  }

  return Promise.all(batches.map((amt) => createQuote({ amount: amt })));
}
```

</Accordion>

<Accordion title="DAILY_VOLUME_LIMIT_EXCEEDED">
**Cause:** The requested amount would exceed the daily volume limit for the trade corridor or withdrawal currency (HTTP 429)

**Solution:** Wait until the next day or reduce the transaction amount

```javascript
async function createQuoteWithLimitCheck(params) {
  try {
    return await createQuote(params);
  } catch (error) {
    if (error.code === "DAILY_VOLUME_LIMIT_EXCEEDED") {
      // Notify user about daily limit
      return {
        error: "DAILY_VOLUME_LIMIT_EXCEEDED",
        message: "Daily transfer limit reached. Try again tomorrow.",
        retryAfter: getNextDayMidnight(),
      };
    }
    throw error;
  }
...
async function createQuoteWithRetry(params, maxRetries = 3) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // Rate limiting (429 with RATE_LIMITED) is retryable with backoff
      // Volume limits (DAILY_VOLUME_LIMIT_EXCEEDED) are NOT retryable
      const isRateLimited =
...
function getUserFriendlyMessage(error) {
  const errorMessages = {
    ...
    TRANSACTION_SIZE_LIMIT_EXCEEDED:
      "This amount exceeds the maximum single transaction limit. Try a smaller amount.",
