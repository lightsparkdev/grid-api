...
<Info>
{rewardsProductName} has two customer onboarding options—one for non-regulated entities where Lightspark handles the KYC/KYB process via a hosted flow, and one for regulated entities where you handle the KYC/KYB process and can create users directly via the API.
</Info>
