...
- **Simplified KYC**: KYC processes are simulated and complete instantly. You must add users via the `/customers` endpoint, rather than using the KYC link flow.
...
- Review [Webhooks](/payouts/platform-tools/webhooks) for event handling
- Check out the [Postman Collection](/payouts/platform-tools/postman-collection) for API examples
- See [Error Handling](/payouts/payment-flow/error-handling) for production error strategies
