...
same-business-day settlement for a USD payout. `ACH` continues to settle on the
standard next-business-day schedule.
...
- A same-day request above the NACHA per-entry limit is rejected at quote time,
  rather than silently settling next day.
