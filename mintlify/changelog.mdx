...
## `BULK_SETTLEMENT` internal accounts for platforms

`InternalAccountType` gains `BULK_SETTLEMENT`. A bulk settlement account is a
platform-owned USD account with its own account number, for receiving a bulk
payment that the platform then pays out to its customers. Only a platform can
hold one, so it has no `customerId`. Funds in it can move only to the
platform's customers, so a quote from it to another platform-owned account is
rejected. Lightspark provisions it on request, and
`GET /platform/internal-accounts?type=BULK_SETTLEMENT` returns it.
