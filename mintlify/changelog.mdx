...
## A card is funded by one account, `fundingSource`

The `fundingSources` array is replaced by a single `fundingSource` string on the
`Card` resource, on `POST /cards`, and on `PATCH /cards/{id}`.

<Warning>
  Breaking change with no alias. Send `fundingSource` as a string; an array is
  rejected. Update your card create and update calls before the new version
  ships.
</Warning>

- The docs described an ordered array that Authorization Decisioning would try
  in priority order. It never did — a card draws on one account, and a failed
  hold does not fall through to another. The contract now says so.
- `PATCH /cards/{id}` takes a `fundingSource` string that replaces the bound
  account. It still cannot be supplied alongside `state: CLOSED`.
- `GET /cards?accountId=` returns cards whose `fundingSource` is that account,
  rather than cards whose array contained it.
- Grid CLI: `--funding-sources <list>` becomes `--funding-source <id>` on
  `grid cards create` and `grid cards update`.

See [Funding source](/cards/card-management/funding-sources).
...
[funding source](/cards/card-management/funding-sources), and
