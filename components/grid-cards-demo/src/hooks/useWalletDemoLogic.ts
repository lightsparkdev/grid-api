...
// Matches CREATING_MS in apps/shared/wallet/useWalletHome: the webhook arrives
// as the card flips from "creating" to "ready" on the phone.
...
export function useWalletDemoLogic() {
  ...
  const onCardIssued = useCallback(() => {
    // POST /cards lands now (card is PROCESSING); the CARD.STATE_CHANGE webhook
    // lands when the on-phone issuance animation finishes and the card is ACTIVE.
    const gid = newGroupId();
    const [createCall, ...activationCalls] = cardCalls();
    pushCalls([createCall], 'Issue a card', gid);
    const timer = setTimeout(() => {
      settleTimers.current.delete(timer);
      pushCalls(activationCalls, 'Issue a card', gid);
