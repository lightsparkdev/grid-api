...
/**
 * Replace `from` in `source` and throw if nothing changed, so an upstream edit
 * to the Cards source fails the check loudly instead of no-op'ing.
 */
function mustReplace(source, from, to, label) {
  const next = source.replace(from, to);
  if (next === source) {
    throw new Error(`Chrome parity transform did not apply: ${label}`);
  }
  return next;
}
...
const exceptions = new Map([
  [
    'next.config.mjs',
    (source) =>
      // Cards redirects its share domain to the docs. Statements has no share
      // domain, so the redirect block is stripped.
      mustReplace(
        source,
        /\n  async redirects\(\) \{\n(?:.*\n)*?  \},\n/,
        '\n',
        'next.config.mjs: strip cards.lightspark.com host redirect',
      ),
  ],
  [
    'src/app/page.module.scss',
...
`,
  ...
  [
    ...
    (source) => {
      ...
      // Cards preloads its card-face assets from the head. Statements has no
      // card face, so the constant and the preload links are stripped.
      next = mustReplace(
        next,
        /\/\*\* The files facePaint\.loadFaceAssets fetches[^\n]*\nconst CARD_FACE_ASSETS = \[\n(?:.*\n)*?\];\n\n/,
        '',
        'layout.tsx: strip CARD_FACE_ASSETS constant',
      );
      next = mustReplace(
        next,
        /        \{\/\* The card's face assets \(facePaint\.loadFaceAssets\)\.(?:.*\n)*?        \)\)\}\n/,
        '',
        'layout.tsx: strip card-face asset preloads',
      );
      return next;
    },
  ],
