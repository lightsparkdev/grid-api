...
const underStepVariants = {
  enter: ({ reduceMotion, sheetReveal }: NavDir) =>
    reduceMotion || sheetReveal
      ? { x: 0, opacity: 1 }
      : { x: -ONDEMAND_PUSH_PARALLAX, opacity: 1 },
  center: { x: 0, opacity: 1 },
  exit: ({ reduceMotion }: NavDir) =>
    reduceMotion ? { opacity: 0 } : { x: -ONDEMAND_PUSH_PARALLAX, opacity: 1 },
};
...
const navMidVariants = {
  enter: ({ back, reduceMotion }: NavDir) =>
    reduceMotion
      ? { x: 0, opacity: 1 }
      : back
        ? { x: -ONDEMAND_PUSH_PARALLAX, boxShadow: EDGE_SHADOW_ON, opacity: 1 }
        : { x: '100%', boxShadow: EDGE_SHADOW_OFF, opacity: 1 },
  center: { x: 0, boxShadow: EDGE_SHADOW_ON, opacity: 1 },
...
const SOURCE_COPY: Record<string, { Icon: OndemandIcon; title: string; sub: string }> = {
  bank: { Icon: IconBank, title: 'Bank account', sub: 'Local transfer in 65+ countries' },
  crypto: { Icon: IconWallet1, title: 'Crypto wallet', sub: 'Spark, Solana, Base address' },
...
const SEND_SOURCE_COPY: Record<string, { Icon: OndemandIcon; title: string; sub: string }> = {
...
/** Steps that have a ondemand face so far — a source row only navigates when
...
/**
 * Ondemand deposit flow — an iOS navigation PUSH, not a sheet: the page
 * slides in from the right over the wallet (left-edge drop shadow riding the
 * slide) while the wallet parallax-shifts behind it (see OndemandWalletScreen).
...
export function AddMoneyPage({
  ...
  // The brain steps with a ondemand face. Sheet-presented steps (country /
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(
      () => setRiseReady(true),
      (ONDEMAND_SHEET_DURATION + 0.15) * 1000,
    );
                        ...
                        {(isSend ? m.banks : banks).map((b) =>
                          'address' in b ? (
                            ...
                            <button
                              ...
                              onClick={() => m.selectBank(b.id)}
                            >
                              <CircleFlag code={b.country.code} />
                              <span className={styles.rowLines}>
                                <span className={styles.rowTitle}>
...
function AmountStep({
  ...
  const destClip = useSquircleClip<HTMLButtonElement>({ figmaRadii: 8 });
...
/**
 * "Select bank" empty state — the Z/Aurora reveal on ondemand chrome:
    ...
    <div className={styles.empty}>
      ...
      <div className={styles.emptyMessageLayer}>
        ...
        >
          ...
          <DarkCta onClick={onAddBank}>{cta}</DarkCta>
        </motion.div>
      </div>
    </div>
  );
